...
const mockStore: ToolStore = {
	...
	search: vi.fn().mockImplementation(async (embedding, _count, threshold) => {
		...
		if (embedding[0] > (threshold || 0)) return [weatherToolDef]

		// simulate finding news tool if embedding[1] is high
		if (embedding[1] > (threshold || 0)) return [newsToolDef]
	...
	TransformersEmbeddingProvider: {
		create: vi.fn().mockResolvedValue({
			getFloatEmbedding: vi.fn().mockImplementation(async (query: string) => {
				if (query.includes('weather')) return [0.9, 0.1]
				if (query.includes('news')) return [0.1, 0.9]
				return [0, 0]
			}),
...
describe('ToolRetriever', () => {
	it('should retrieve tools based on semantic similarity', async () => {
		const mockEmbeddingProvider = await (
			await import('../providers/embedding/transformers')
		).TransformersEmbeddingProvider.create()
		const retriever = await ToolRetriever.create({
			tools: [weatherToolDef, newsToolDef],
			store: mockStore,
	...
	describe('strict mode', () => {
		...
		it('should throw an error for a missing explicit tool when strict is true', async () => {
			...
			await expect(retriever.retrieve(query, { strict: true })).rejects.toSatisfy(
				(e: ToolNotFoundError) => e.toolName === 'aMissingTool',
			)
		})
